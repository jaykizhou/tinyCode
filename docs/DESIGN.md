...
| 局限                         | 说明                                                     | 演进方向                                         |
|-----------------------------|---------------------------------------------------------|-------------------------------------------------|
| 不支持流式响应               | `Complete` 是一次性返回                                   | 在 `Model` 接口新增 `Stream` 方法，Agent 做增量渲染|
| 无会话压缩                   | 长对话会把历史全量发给模型                                 | 参考 blades-main `context/summary` 做滚动压缩      |
| 工具顺序执行                 | 同一轮多个 tool_calls 顺序跑                              | 在 `executeTool` 用 `errgroup` 并发               |
| 黑名单基于子串               | 无法表达"禁止 rm -rf 但允许 rm 单个文件"                    | 升级为正则 / AST 级规则引擎                        |
| 无中间件                    | 日志/重试/审批硬编码                                      | 参考 blades-main `middleware` 模式做链式包装        |
| 单工具                     | 仅 Shell                                                | 新增 read_file / write_file / http 等（接口已就绪）  |
| REPL 单线程                 | 长命令期间无法接收新输入                                   | 引入 tview / readline 做全屏交互                  |
